 ...
 *
 * v006 (network-evidence round): payloads can now carry `failedRequests` — the
 * 4xx/5xx responses recorded in the failing attempt's trace. Until this round the
 * server-error provenance rule and the batch-wide-outage rule could only fire when
 * a status appeared in the error TEXT, which happens only when a test's own API
 * helper throws it. A UI-side failure never says so: a backend 503 surfaces as
 * "Timeout 60000ms exceeded while waiting on the predicate" and reads exactly like
 * a flake, and a dogfood run classified such a failure ENV_ISSUE only by inferring
 * backend trouble from the shape of the wait — correct, but a guess the evidence
 * did not have to leave to chance. v006 names the field in the taxonomy, extends
 * the server-error provenance rule to it, and adds 429 (rate limiting) to the
 * environment statuses.
 */
export const PROMPT_VERSION = 'v006';
...
- failedRequests rule: \`failedRequests\` lists the 4xx/5xx responses captured in the failing attempt's trace (deduplicated, query strings stripped). It is the ONLY place a UI-side failure reveals a backend status — the error text of such a failure describes a wait, not a status — so read it before deciding a timeout is a race. Weigh it with the server-error provenance rule: a 5xx or 429 on a call the test does not assert on (catalog, permissions, context fetch, seed) makes the failure ENV_ISSUE even when the error text mentions only a timeout, an empty list, or a missing element. A 429 is rate limiting: environment load, never an app defect. Quote the offending status and endpoint in "why". The field is ABSENT when the run captured no trace (tracing is opt-in), and its absence is not evidence that the backend was healthy — fall back to the other rules rather than reading it as a clean bill of health.
