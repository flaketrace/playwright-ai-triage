...
- b201b0c: Flag stalled attempts inside the retry-then-passed FLAKY verdict.
...
- c15c2e9: Give the classifier the backend status behind a UI failure.
...
- 950d07a: Stable classifications on PRs: persisting failures reuse the class recorded in the reporter's
...
- 05ec912: New opt-in HTTP sink: set the `sinkUrl` option (or `AI_TRIAGE_SINK_URL`) to POST each run's
